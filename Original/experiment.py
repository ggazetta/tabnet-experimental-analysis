...
def main(unused_argv):
  ...
  tabnet_forest_covertype = tabnet_model.TabNet(
      columns=data_helper.get_columns(),
      num_features=data_helper.NUM_FEATURES,
      feature_dim=128,
      output_dim=16,
  ...
  column_names = sorted(data_helper.FEATURE_COLUMNS)
  print(
  ...
  train_batch = data_helper.input_fn(
      TRAIN_FILE, num_epochs=50000, shuffle=True, batch_size=BATCH_SIZE)
  val_batch = data_helper.input_fn(
      VAL_FILE,
      num_epochs=5000,
      shuffle=False,
      batch_size=data_helper.N_VAL_SAMPLES)
  test_batch = data_helper.input_fn(
      TEST_FILE,
      num_epochs=5000,
      shuffle=False,
      batch_size=data_helper.N_TEST_SAMPLES)
